...
def forecast_object_to_dict(forecast_object):
    ...
    dict_ = zip(forecast_object.names, list(forecast_object))
    dict_ = dict(dict_)
...
class ForecastModel(BaseEstimator, RegressorMixin):
    ...
    def fit(self, y):
        self.fitted_model_ = fit_forecast_model(
            y, self.freq, self.model, **self.kwargs
        )
...
class ForecastObject(BaseEstimator, RegressorMixin):
    ...
    def predict(self, h):
        ...
        fitted_model = fit_forecast_model(
            self.y_ts_, self.freq, self.model, h=h, **self.kwargs
        )
...
class TBATS(ForecastModel):
    ...
    def __init__(self, freq, **kwargs):
        super().__init__(
            model="tbats", freq=freq, **{"use.parallel": False}, **kwargs
        )
